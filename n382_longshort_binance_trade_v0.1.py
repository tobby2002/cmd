...
with open('config_trade.json', 'r') as f:
    config = json.load(f)
...
def blesstrade(df, symbol, fcnt, longshort, df_lows_plot, df_highs_plot, wavepattern, trade_info, idx, wavepattern_l, wavepattern_tpsl_l):
    t = trade_info
    stats_history = t[0]
    order_history = t[1]
    asset_history = t[2]
    ...
    # target_price = entry_price + height_price * 0.618 if longshort else entry_price - height_price * 0.618  # 0.5 되돌림가정
    # target_price = entry_price + height_price * 0.382 if longshort else entry_price - height_price * 0.382  # 0.5 되돌림가정
        ...
        for i, close in enumerate(closes):
            ...
            if position is True:
                if c_profit or c_stoploss:
                    ...
                    print(symbol, fcnt, 'L' if longshort else 'S', trade_inout_i[0][0][2:-3], str(trade_stats))
                            ...
                            if wavepattern.check_rule(rule):
                                if wavepattern in wavepatterns:
                                    ...
                                    if not check_has_same_wavepattern(symbol, fc, wavepattern_l, wavepattern):
                                        ...
                                        trade_info = blesstrade(df_all, symbol, fc, longshort, df_lows_plot, df_highs_plot, wavepattern, trade_info, i, wavepattern_l, wavepattern_tpsl_l)
